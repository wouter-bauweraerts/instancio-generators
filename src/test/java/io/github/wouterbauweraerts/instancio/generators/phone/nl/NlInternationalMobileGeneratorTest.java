...
class NlInternationalMobileGeneratorTest {
    private static final Pattern NL_INTERNATIONAL_MOBILE_PATTERN = Pattern.compile("^\\+31 6 [0-9]{2} [0-9]{2} [0-9]{2}$");

    @Test
    void generateReturnsStringInExpectedFormat() {
        String phone = Instancio.of(PhoneWrapper.class)
                .generate(field(PhoneWrapper::getPhone), Generators.phone().nl().international().mobile())
                .create()
                .getPhone();

        assertThat(NL_INTERNATIONAL_MOBILE_PATTERN.matcher(phone)).matches();
    }
